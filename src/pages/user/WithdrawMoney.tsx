...
const WithdrawMoney = () => {
    ...
    const { data } = useGetAgentsQuery({ searchTerm });
    ...
    const onSubmit = async (values: z.infer<typeof withdrawSchema>) => {
        ...
        try {
            const res = await withdrawMoney(info).unwrap();
    ...
    return (
        <div className="p-4 sm:p-6">
            <h1 className="text-2xl font-bold text-center mb-10">Withdraw Money</h1>
                            ...
                            render={({ field }) => (
                                <FormItem>
                                    ...
                                    <div className="flex gap-2">
                                        <FormControl>
                                            <Input placeholder="Enter agent's email" {...field} />
                                        </FormControl>
                                            ...
                                            <DialogContent className="max-w-md flex flex-col">
                                                <DialogHeader>
                                                    ...
                                                    <DialogDescription className="sr-only">
                                                        Choose an agent from the list below to auto-fill the email field.
                                                    </DialogDescription>
                                                </DialogHeader>
                                                ...
                                                <div className="flex-1 max-h-60 overflow-y-auto space-y-2">
                                                    {data?.data.length > 0 ? (
                                                        data?.data.map((agent: IUser) => (
                                                            <div
                                                                key={agent._id}
                                                                className={`p-2 border rounded-lg cursor-pointer hover:bg-accent ${selectedEmail === agent.email ? "bg-accent" : ""
                                                                    }`}
                                                                onClick={() => setSelectedEmail(agent.email)}
                                                            >
                                                                <p>{agent.name}</p>
